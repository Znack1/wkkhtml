...
import applyStyleFunction, {getValue} from './MBStyleToOLStyle';
...
export function applyStyle(layer, glStyle, source, path, resolutions) {
  return new Promise(function(resolve, reject) {
    ...
    let layerType= typeof(layer);
//import VectorLayer from 'ol/layer/Vector';
    //

    if (!(layer instanceof ol.layer.Vector || layer instanceof ol.layer.VectorTile || layer instanceof VectorLayer || layer instanceof VectorTileLayer)) {
 ...
 * argument.
 */
export default function ApplyMBStyleForOLLayer(map, style) {
...
export function apply(map, style) {
  ...
  setTimeout(function() {
    ApplyMBStyleForOLLayer(map, style);
  }, 0);
  return map;
}
